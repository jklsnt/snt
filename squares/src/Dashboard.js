...
function Dashboard({ logOut }) {
    ...
    <div className="fixed relative flex flex-col w-screen h-screen overflow-x-hidden bg-gray-900 select-none">
        { is_complete ? <div className="fixed"><Confetti numberOfPieces={400} /></div> : <></> }
        ...
        <div className="w-full">
            <div className="w-5/6 m-auto mt-8 font-mono text-center text-gray-600 bg-gray-800 rounded-2xl" style={{maxWidth: 507+'px'}}>
                <SquaresDisplay squares_data={squares}></SquaresDisplay>
                the beginning of time
            </div>
        </div>
        <div className="fixed bottom-0 flex flex-row-reverse w-full h-20 p-10">
            <button onClick={logOut} className="text-white bg-gray-800 rounded-lg" style={{ padding: "10px" }}>logout <LogoutIcon /></button>
        </div>
    </div>
    );
}
